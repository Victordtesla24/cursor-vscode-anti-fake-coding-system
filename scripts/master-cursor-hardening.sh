...
MASTER_LOG_FILE="$(pwd)/logs/master/cursor-hardening.log"
readonly MASTER_LOG_FILE
...
log() {
    ...
    echo "[$ts] [$MASTER_SCRIPT_NAME] [$level] $message" | tee -a "$MASTER_LOG_FILE" >/dev/null 2>&1 || true
}
...
main() {
    # Initialize log file
    mkdir -p "$(dirname "$MASTER_LOG_FILE")" 2>/dev/null || true
    touch "$MASTER_LOG_FILE" 2>/dev/null || true
    chmod 644 "$MASTER_LOG_FILE" 2>/dev/null || true
